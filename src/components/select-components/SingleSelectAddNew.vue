<template>
    <div ref="vuesingleselect">
        <div v-if="!selectedOption" :class="classes.wrapper">
            <div @click="changeDropdownState" @keyup="changeDropdownState" class="relative inline-block w-full">
                ...
                <ul tabindex="-1" ref="options"
                    :style="{'max-height': maxHeight}" style="z-index: 100;"
                    :class=[classes.dropdown]
                    class="absolute w-full overflow-auto appearance-none mt-px  list-reset"
                >
                    <template v-if="searchText && showDropdown">
                        <li
                            class="cursor-pointer outline-none we-select-add-new"
                            @blur="handleClickOutside($event)"
                            @keyup.down="addPointerDown"
                            @click.prevent="insertNew(searchText)"
                        >
                            Add {{searchText}}
                        </li>
                    </template>
                    <li tabindex="-1" v-if="matchingOptions"
    ...
    export default {
        ...
        methods: {
            ...
            insertNew( newOption ) {
                ...
                this.$emit( 'clickedAddNew', newOption );
            },
...
<style scoped>
    ...
    .we-select-add-new:before {
        content: "+";
    }

    .we-select-add-new:hover {
        background: #dae1e7;
    }
</style>
